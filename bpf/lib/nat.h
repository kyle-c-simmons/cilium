...
static __always_inline int
__snat_v4_nat(struct __ctx_buff *ctx, struct ipv4_ct_tuple *tuple, __s64 fraginfo,
...
snat_v4_nat(struct __ctx_buff *ctx, struct ipv4_ct_tuple *tuple,
	    struct iphdr *ip4, __s64 fraginfo,
	    ...
	    struct trace_ctx *trace, __s8 *ext_err)
{
	...
	/* If we don't track fragments, packets without an L4 header can't be
	 * NATed. Even though the first fragment always has an L4 header, NATing
	 * it in this situation is useless, because the following fragments
	 * won't be able to pass the NAT.
	 */
